...
import { X, Briefcase, Trello, UserPlus } from "lucide-react";
        ...
        <form onSubmit={handleFormSubmit} className="p-6 space-y-4">
          ...
          {type !== "invite" ? (
            <>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">
                  {type === "workspace" ? "Workspace Name" : "Board Name"}
                </label>
                <input
                  type="text"
                  required
                  placeholder={type === "workspace" ? "e.g. Design & Development" : "e.g. Sprint 1 Kanban"}
                  value={name}
            ...
            <div className="space-y-4">
              <div className="text-xs text-slate-500 leading-normal">
                Invite registered users to <span className="font-bold text-slate-700">{activeWorkspace?.name}</span> to collaborate.
              </div>
              ...
              <div className="space-y-1.5">
                ...
                {inviteable.length > 0 ? (
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-sm text-slate-700 font-semibold focus:outline-none focus:border-indigo-500 transition shadow-xs cursor-pointer"
                  >
          ...
          <div className="flex gap-2.5 justify-end pt-3 border-t border-slate-100">
            ...
            <button
              ...
              className="px-5 py-2 text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/20 active:scale-[0.98] transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            >
