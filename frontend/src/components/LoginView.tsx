...
export default function LoginView({ onLoginSuccess, availableUsers }: LoginViewProps) {
  // If no users exist yet in database, start on Register screen
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden z-10 transition-all duration-300">
        ...
        <div className="px-8 pt-8 pb-4 text-center">
          <div className="inline-flex items-center gap-2 mb-3">
            <div className="bg-gradient-to-tr from-indigo-500 to-sky-400 p-2.5 rounded-xl text-white shadow-lg shadow-indigo-500/20">
          ...
          <p className="text-xs text-slate-400 mt-1.5">
            {isRegistering
              ? "Register a new dynamic account to get started"
              : "Sign in to access your workspaces and boards"}
          </p>
        </div>
        ...
        <div className="p-8 pt-4">
          ...
          {!isRegistering ? (
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full mt-2 bg-indigo-600 hover:bg-indigo-500 text-white font-medium text-sm py-2.5 px-4 rounded-xl shadow-lg shadow-indigo-600/10 active:scale-[0.98] transition flex items-center justify-center gap-2 disabled:opacity-50 disabled:pointer-events-none cursor-pointer"
              >
                {loading ? (
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    <option value="Teacher">Teacher (Full Access & Management)</option>
                    <option value="Student">Student (Activity & Task Tracking)</option>
                  </select>
          ...
          {/* Quick login for registered users (only if actual users exist) */}
          {!isRegistering && availableUsers.length > 0 && (
            <div className="mt-6 pt-5 border-t border-slate-800/80">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest text-center mb-3">
                Quick Sign-In (Select Existing User)
              </p>
              <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto pr-1">
                {availableUsers.map((user) => (
                  <button
                    ...
                    className="flex flex-col items-start p-2.5 bg-slate-950 hover:bg-slate-800/40 border border-slate-800/60 hover:border-slate-700/80 rounded-xl transition text-left group cursor-pointer"
                  >
          ...
          <div className="mt-6 text-center">
            <button
              ...
              className="text-xs text-slate-400 hover:text-indigo-400 hover:underline transition font-medium cursor-pointer"
            >
              {isRegistering
